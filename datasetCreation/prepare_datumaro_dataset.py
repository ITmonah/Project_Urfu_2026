...
def clear_output_dirs(output_dir: Path):
    for class_name in STATE_LABELS:
        for image_path in (output_dir / class_name).glob("*.jpg"):
            image_path.unlink()
        ...
        for item in annotation_payload.get("items", []):
            ...
            for crop_idx, (x, y, width, height) in enumerate(target_boxes):
                ...
                image.crop((x1, y1, x2, y2)).save(
                    class_dir / f"{zip_stem}__{item_prefix}__crop_{crop_idx}.jpg",
                    quality=95,
                )
                saved[state_name] += 1
...
def build_summary(output_dir: Path, archive_summaries: dict):
    summary = {
        ...
        "total_by_class": {class_name: len(list((output_dir / class_name).glob("*.jpg"))) for class_name in STATE_LABELS},
    }
